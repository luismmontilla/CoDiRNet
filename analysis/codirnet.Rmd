...
#07a. individual community plots

Each graph are assigned to a g.comm# where "#" is the number of the community

```{r individual community plots}

for(i in 1:length(levels(lc$edges$cluster))) {
  assign(paste("comm", levels(lc$edges$cluster)[i], sep = ""),
         lc$edges[lc$edges[, 3] == i, ])
  
  assign(paste("g.", "comm", levels(lc$edges$cluster)[i], sep = ""),
         ggnet2(graph.data.frame(get(paste("comm", levels(lc$edges$cluster)[i],
                                           sep = ""))), label = T))
}
